...
with requests.get(url) as response:
    data = response.json()
    # Two different ways of composing text messages. The format
...
#
# Based on the results of the analysis presented in Lab 2, we only
# need to deal with the 'properties' dictionaries within 'data'. This
# is a relatively small part of the data file, since most of the data
# is geometric coordinates. Therefore, the first step is to make a
# list of just the 'properties' dictionaries.
...
# We no longer need to store the whole input file in 'data', but I
# have left this commented for convenience del(data)

# ### Aggregating the counties

# First, I define a function called aggDict() that takes a list of
# dictionaries that have at least one key in common. It returns a new
# dictionary where the keys are all the unique values of the selected
# common key.  For example, if we use aggDict() on the list of
# counties dictionaries using the 'STATE' key, the function will
# return a dict with one key for each state name. The value of each
# key will be a list of all the counties in that state (see the
# analysis by state below).
...
#
# Next, we'll make a function that uses an aggegated dictionary to
# return a sorted list of the n keys with the most counties.
#
# Taking our state example above, we could use this function with the
# dictionary aggregated by state to return the five states with the
# most counties.
...
# Next, let's make a function to visually display the n most frequent
# keys in an aggregated dictionary.
...
def aggDictPie(agg_dict: dict, nameAlias=lambda x: x):
    ...
    pie = plotly.pie(plot_dict, names="names", values="quantity", hole=0.3)
    pie.show()
...
for county_name in common_county_names:
    ...
    print(
        f"There are {len(county_dict[county_name])} {county_name} Counties, one each in the following states:\n {[state_codes[county['STATE']] for county in county_dict[county_name]]} \n"
    )
...
# We can see that there are 3,007 entries of type "County," as we
# would expect, but more than a hundred entries with other LSAD
# values. We would have to know more about the dataset to understand
# why, but it's interesting to note that not all of the putative
# counties are listed as such.

# ## State-by-state analysis
#
# Next, we will examine state-level statistics. We can apply the same
# functions as above but aggregate on state code rather than county
# name.
#
# We can also see the top 5 states with the most counties using the
...
print(
    f"The five states with the most counties are: {[state_codes[state] for state in frequentValues(state_dict, 5)]})"
)
...
for state_code in frequentValues(
    state_dict, -1
):  # with n=-1, frequentValues() gives a list of all the  keys sorted by frequency
    ...
    output_file.write(
        f"The mean area of the {num_counties} counties in {state_codes[state_code]} is {round(mean_area, 2)}.\n"
    )
    output_file.write(
        f"The smallest is {smallest_county['NAME']} County, with an area of {smallest_county['CENSUSAREA']}.\n"
    )
    output_file.write(
        f"The largest is {biggest_county['NAME']} County, with an area of {biggest_county['CENSUSAREA']}.\n\n"
    )
...
# ### Plotting the aggregated dictionaries Finally, we can use the
# aggDictPlot() function to take a look at dictionaries we've made so
# far.  First, let's look at the top 20 most frequent county names.
# Note that the plotting function properly capitalizes 'county' even
# though it is lowercase in the function call.

aggDictPlot(county_dict, "county", save=True, n=20)


# Next, we can do the same for states, using the default number of
# elements to plot and the stateName function to get the state names
...
# Finally, in the case of the LSAD function, the plot can tell that
# all of the elements are plotted, so the title changes accordingly.
# Also note that the plotting function corrects the lowercase "a" in
# the function call for "area" below.
#
